...
│   │
│   └── evaluate.py       # RAGAS evaluation script — runs 10 test queries through
│                         # the full pipeline and scores faithfulness, answer
│                         # relevancy, and context relevance; saves ragas_results.csv
│
├── data/
│   ├── manuals/
...
├── chunks_cache.pkl      # Cached document chunks — auto-generated on first run, avoids re-parsing PDFs
├── ragas_results.csv     # RAGAS evaluation output — generated by evaluate.py
│
...
<h3>1. Clone and create environment</h3>

**Python 3.11 is required.** Python 3.13 has a known issue where streamlit hangs on startup due to protobuf C extension incompatibility.
...
On the first run, the app will read all PDFs, split them into 1000-character chunks with 200-character overlap, embed each chunk using text-embedding-3-small, and save two FAISS indexes and a `chunks_cache.pkl` to disk. This takes 1-2 minutes depending on how many PDFs you have. Every run after that loads directly from the saved cache — no PDF parsing or re-embedding.

If you add new PDFs later, delete `faiss_index/`, `faiss_obd_index/`, and `chunks_cache.pkl` so everything rebuilds with the new content included.
...
- Honda Civic 2011 brake pedal feels soft and spongy
- Toyota Camry 2018 P0171 system too lean bank 1
- Ford F-150 2018 transmission jerks when shifting into drive
- Toyota Corolla 2019 grinding noise when turning
...
The project includes a RAGAS-based evaluation script that measures answer quality across three dimensions:

**Faithfulness** — checks that every claim in the answer is grounded in the retrieved context. A score of 1.0 means no hallucination; the LLM only stated things that were present in the retrieved documents.

**Answer Relevancy** — measures how directly the answer addresses the question asked. High scores mean the answer is focused and on-topic; low scores indicate the response drifted or was vague.

**Context Relevance** — measures how relevant the retrieved chunks were to the question. A high score means retrieval surfaced the right sections of the manuals.

The script runs 10 representative test questions through the full HyDE → retrieval → generation pipeline, scores each with all three metrics, prints per-question and aggregate results, and saves everything to `ragas_results.csv` in the project root.
...
The evaluation uses RAGAS 0.4.x with the collections metrics API. Each metric is scored per sample individually rather than using `ragas.evaluate()`.
...
First load is slow — embedding thousands of PDF chunks takes time depending on file size and count. After the first run the indexes and document chunks are saved to disk and every subsequent load is instant.
...
- RAGAS — RAG evaluation framework (faithfulness, answer relevancy, context relevance)
- pandas — tabular results output for evaluation scores
